...
import {
  getUtangUsaha,
} from "@/services/mahasiswa/tugas/audit/utang_usaha/utang_usaha";

import useProsedur from "@/hooks/mahasiswa/tugas/audit/pengujian_substantif/utang_usaha/prosedur/use_prosedur";
...
export default function ProsedurTab({ auditId }) {
  ...
  const [isUtangUsahaLoading, setIsUtangUsahaLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchUtangUsaha = async () => {
      if (!auditId) {
        setIsUtangUsahaLoading(false);
        return;
      }

      try {
        setIsUtangUsahaLoading(true);

        const data = await getUtangUsaha(auditId);

        console.log("DATA UTANG USAHA:", data);

        setUtangUsahaId(data?.UtangUsahaID ?? null);
        setKesimpulan(data?.Kesimpulan ?? "");
      } catch (error) {
        console.error("Gagal mengambil data utang usaha:", error);
  ...
  if (!utangUsahaId) {
    return (
      <div className="py-10 text-center">
        <span className="font-poppins text-sm text-[#64748B]">
          Data utang usaha tidak ditemukan.
        </span>
      </div>
    );
  }
