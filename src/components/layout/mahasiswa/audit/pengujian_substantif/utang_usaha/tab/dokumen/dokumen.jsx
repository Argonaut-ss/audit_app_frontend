...
import useDokumen from "@/hooks/mahasiswa/tugas/audit/pengujian_substantif/utang_usaha/dokumen/use_dokumen";
...
export default function DokumenTab({ auditId }) {
  ...
  useEffect(() => {
    const fetchUtangUsaha = async () => {
      ...
      try {
        const response = await getUtangUsaha(auditId);

        const utangUsaha = response ?? null;
