...
export default function MutasiStockOpnameForm({
  isOpen,
  onClose,
  onSubmit,
  isSubmitting = false,
  ...
  const handleNamaFileChange = (event) => {
    setFormData((prev) => ({
      ...prev,
      namaFile: event.target.value,
    }));
  };
  ...
  const handleSubmit = async (event) => {
    ...
    await onSubmit({
      namaFile: formData.namaFile.trim(),
      file: formData.file,
    });
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-poppins text-xl font-semibold text-white">
                {mode === "edit"
                  ? "Edit Mutasi Stock Opname"
                  : "Input Mutasi Stock Opname"}
              </h2>

              <p className="mt-1 font-poppins text-sm text-white">
                {mode === "edit"
                  ? "Perbarui dokumen mutasi stock opname"
                  : "Tambahkan dokumen mutasi stock opname"}
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-6 px-8 py-7">
            ...
            <div>
              ...
              <div className="flex h-11 items-center rounded-lg border border-[#DCE5EF] bg-white transition focus-within:border-[#38BDF8]">
                <input
                  ...
                  disabled={isSubmitting}
                  className="min-w-0 flex-1 bg-transparent px-4 font-poppins text-sm text-[#475569] outline-none placeholder:text-[#94A3B8] disabled:cursor-not-allowed disabled:opacity-60"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <button
                ...
                disabled={isSubmitting}
                className="flex min-h-[68px] w-full items-center gap-4 rounded-xl border border-dashed border-[#CBD5E1] bg-white px-5 text-left transition hover:border-[#38BDF8] hover:bg-[#F8FCFF] disabled:cursor-not-allowed disabled:opacity-60"
              >
                ...
                <div className="min-w-0">
                  {formData.file ? (
                    ...
                    <>
                      ...
                      <p className="mt-1 font-poppins text-xs text-[#94A3B8]">
                        PDF, DOC, DOCX, XLS, XLSX, JPG, JPEG, PNG · Maks. 16 MB
                      </p>
                    </>
                  )}
                </div>
          ...
          <div className="flex justify-end gap-3 px-8 py-6">
            <button
              ...
              onClick={onClose}
              disabled={isSubmitting}
              className="min-w-[95px] rounded-lg border border-[#CBD5E1] bg-white px-6 py-3 font-poppins text-sm font-bold text-[#475569] transition hover:bg-[#F8FAFC] disabled:cursor-not-allowed disabled:opacity-60"
            >
              Batal
            </button>
            ...
            <button
              type="submit"
              disabled={!canSubmit}
              className="min-w-[120px] rounded-lg bg-[#05A80B] px-6 py-3 font-poppins text-sm font-bold text-white transition hover:bg-[#04930A] disabled:cursor-not-allowed disabled:opacity-50"
            >
