...
import {
  ...
  getDokumenById,
} from "@/services/mahasiswa/tugas/audit/persediaan/dokumen/dokumen";

export default function useDokumen({ persediaanId }) {
  ...
  const fetchDokumen = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        const response = await getDokumen(persediaanId, page);
  ...
  const addDokumen = async (data) => {
    ...
    const response = await createDokumen(persediaanId, data);
