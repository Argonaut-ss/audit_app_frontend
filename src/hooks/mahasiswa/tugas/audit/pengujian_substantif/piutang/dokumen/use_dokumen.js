...
import {
  ...
  getDokumenById,
} from "@/services/mahasiswa/tugas/audit/piutang/dokumen/dokumen";

export default function useDokumen({ piutangId }) {
  ...
  const fetchDokumen = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        const response = await getDokumen(piutangId, page);
  ...
  const addDokumen = async (data) => {
    if (!piutangId) {
      throw new Error("PiutangID tidak tersedia.");
    }

    const response = await createDokumen(piutangId, data);
