...
import {
  ...
  deleteProsedur,
  saveAllProsedur,
} from "@/services/mahasiswa/tugas/audit/persediaan/prosedur/prosedur";

export default function useProsedur({ persediaanId }) {
  ...
  const fetchProsedur = useCallback(async () => {
    ...
    try {
      ...
      const response = await getProsedur(persediaanId);
  ...
  const handleCreate = async (data) => {
    if (!persediaanId) {
      return {
        success: false,
        message: "ID persediaan usaha tidak ditemukan",
      };
    }
